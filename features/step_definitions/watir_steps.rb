# https://github.com/napcs/cucumber_watir/blob/master/features/step_definitions/watir_steps.rb

# Use a syntax like "Then /^I should (not )?see/ do |invert|" and pass the value of invert
# as the flag, the value is the calculated value if the "not" isn't included.
def notable(flag, value)
  (flag == "") ? value : !value
end 

### [urls] When I visit "http://www.twitter.com/"
When /^I visit "(.*)"$/ do |url| 
  @browser.goto(path_to(url))
end
...
Given /I should (not )?see a div with the id "([^\"]*)"$/ do |invert, id|
  notable(invert, @browser.div(:id, id).exists?)
end
